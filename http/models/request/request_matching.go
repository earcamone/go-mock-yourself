...
func (self Request) Match(request *http.Request) bool {
	...
	//
	// NOTE: Requests Ready() method won't return true (disabling the ability to use a Mock Request by Go Mock Yourself)
	// unless at least one request criteria is set thus if non of the matching criterias return false, then the being
	// applied Mock Request has all its set criterias matching, thus is a matching Mock Request.
	//
