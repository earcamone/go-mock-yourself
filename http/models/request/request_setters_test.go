...
func TestRequestsHeadersSettingErrorHandling(t *testing.T) {
	...
	//
	// Test setting failing to compile headers
	//

	failingToCompileHeaders := make(map[string]string)
	failingToCompileHeaders["*"] = "*"

	mockRequest := new(Request)
	mockRequestError := mockRequest.SetHeaders(failingToCompileHeaders)

	//
	// Ensure SetHeaders() returned the corresponding error
	//

	ass.Contains(mockRequestError.Error(), "there was an error attempting to compile the received headers")
}
